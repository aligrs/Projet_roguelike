...
public class NameScreen implements Screen {
    ...
    public void displayOutput(AsciiPanel terminal) {
        ...
        if (name != null && name.length() > 0) terminal.writeCenter("[" + name + "]", 18);
        else terminal.writeCenter("[insert hero name here]", 18);
        terminal.writeCenter("-- press [enter] to continue --", 32);
    }
